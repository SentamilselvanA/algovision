...
export default function BFSViz({ step, graph, variables }) {
  const { current, visited, queue } = variables;
    ...
    <div ref={containerRef} className="relative h-full flex flex-col">
      ...
      <div className="flex justify-center mb-4">
        <div className="bg-dark-card p-4 rounded-lg border border-dark-border">
          <div className="text-sm mb-2">Queue: [{queue?.join(', ') || ''}]</div>
          ...
          return (
            <motion.div
              ...
              className={`absolute w-16 h-16 rounded-full flex items-center justify-center font-bold text-lg border-2
                ${current === parseInt(node) ? 'bg-yellow-500 border-yellow-400 text-black shadow-lg shadow-yellow-500/50' : ''}
